  ...
  <view class="audit-container">
    ...
    <view class="merchant-info">
      ...
      <view class="info-item">
        ...
        <image
          :src="merchantInfo.shopPhoto || '/static/default-avatar.png'"
          class="shop-avatar"
        ></image>
      </view>
      <view class="info-item">
      ...
      <view class="info-item">
        ...
        <image
          :src="
            merchantInfo.businessLicenseImage || '/static/default-license.png'
          "
          class="license-image"
        ></image>
      </view>
    ...
    <!-- 审核操作 -->
    <view
      v-if="merchantInfo.applicationStatus === '待审核'"
      class="audit-actions"
    >
      <button class="action-btn approve" @click="showApproveDialog">
        通过
      </button>
  ...
  methods: {
    ...
    async submitAuditResult(status, comment = "") {
      try {
        this.isLoading = true;
        // 确保 merchantId 是整数
        const id = Number.isInteger(this.merchantId)
          ? this.merchantId
          : parseInt(this.merchantId);
        // 构建 URL 参数
        const queryParams = `id=${id}&applicationStatus=${encodeURIComponent(
          status
        )}&auditComment=${encodeURIComponent(comment)}`;
        // 拼接完整的请求 URL
